...
transforms to:
\begin{align*}
	\lagr' &= c {\psi^\dagger}'(x) \gamma^0 \left[i\hbar\gamma^\mu\pder{}{x^\mu} - mc\right] \psi'(x)	\\
	&= c \psi^\dagger(x) \exp(-i\alpha\gamma_5) \gamma^0 \left[i\hbar\gamma^\mu\pder{}{x^\mu} - mc\right] \exp(i\alpha\gamma_5) \psi(x)
\end{align*}
Note that the first term has two powers of the Dirac $\gamma$-matrix: the $\gamma^0$ originating from the adjoint spinor $\overbar{\psi}$ and the $\gamma^\mu$ in front of the derivative. Hence, the order of the products for the first term can be exchanged such that the two exponential factors due to the chiral phase transformation cancel each other. However, the second term (i.e. the mass term) has only one $\gamma^0$, and thus gains a negative sign. i.e.,
\begin{align*}
	\lagr' &= c \overbar{\psi}(x)\left[i\hbar\gamma^\mu\pder{}{x^\mu} + mc\right]\psi(x)
\end{align*}
...
Note that $\psi_\text{L} + \psi_\text{R} = \psi$ and $\psi_\text{R} - \psi_\text{L} = \gamma_5 \psi$.

Applying the operator $\left(i\hbar\slashed{\partial}-mc\right)$ to these fields gives their equations of motion:
\begin{align*}
	\left(i\hbar\slashed{\partial}-mc\right)\psi_\text{L}(x)
	&= \frac{1}{2}\left(i\hbar\gamma^\mu\pder{}{x^\mu}-mc\right)\left(1-\gamma_5\right)\psi(x)	\\
...
If the mass of the Dirac field $\psi(x)$ was zero, then the equations of motion for $\psi_\text{L}, \psi_\text{R}$ would be decoupled:
\begin{equation*}
...
\begin{align*}
	...
	&= i\hbar c \pder{}{x^\mu}\left(\frac{1}{2}\left(1-\gamma_5\right)\psi^\dagger \gamma^0 \gamma^\mu\right)	\\
	&= - i \hbar c \gamma^\mu \pder{}{x^\mu} \left(\frac{1}{2}\left(1+\gamma_5\right)\psi^\dagger\gamma^0\right)	\\
	&= - i \hbar c \gamma^\mu \pder{}{x^\mu}\overbar{\psi}_\text{R}	\\
	&= 0	\\
\intertext{and}
	\pder{}{x^\mu}\left(\pder{\lagr_\text{L}}{\overbar{\psi}_{\text{L},\mu}}\right) - \pder{\lagr_\text{L}}{\overbar{\psi}_\text{L}}
	&= - i\hbar c \gamma^\mu \partial_\mu \psi_\text{L}(x)	\\
	&= 0
\end{align*}
